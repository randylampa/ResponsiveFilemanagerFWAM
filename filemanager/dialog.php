...
<html>
    ...
    <body>
        ...
        <script src="//cdnjs.cloudflare.com/ajax/libs/blueimp-load-image/5.16.0/load-image.all.min.js"></script>
        ...
        <input type="hidden" id="field_id" value="<?php echo str_replace(['[', ']'], ['\[', '\]'], $field_id); ?>"/>
...
//$prev_folder=array();
            ...
            function filenameSort($x, $y)
            {
                ...
                if ($x['is_dir'] !== $y['is_dir']) {
                    ...
                    $greater = ($descending) ? $x['file_lcase'] < $y['file_lcase'] : $x['file_lcase'] >= $y['file_lcase'];
                }
                return $greater ? 1 : -1;
            }
            ...
            function dateSort($x, $y)
            {
                ...
                if ($x['is_dir'] !== $y['is_dir']) {
                    ...
                    $greater = ($descending) ? $x['date'] < $y['date'] : $x['date'] >= $y['date'];
                }
                return $greater ? 1 : -1;
            }
            ...
            function sizeSort($x, $y)
            {
                ...
                if ($x['is_dir'] !== $y['is_dir']) {
                    ...
                    $greater = ($descending) ? $x['size'] < $y['size'] : $x['size'] >= $y['size'];
                }
                return $greater ? 1 : -1;
            }
            ...
            function extensionSort($x, $y)
            {
                ...
                if ($x['is_dir'] !== $y['is_dir']) {
                    ...
                    $greater = ($descending) ? $x['extension'] < $y['extension'] : $x['extension'] >= $y['extension'];
                }
                return $greater ? 1 : -1;
            }
                            ...
                            <div class="filters">
                                <div class="row-fluid">
                                    <div class="span4 half">
                                        <?php if ($config['upload_files']) { ?>
                                            <button class="tip btn btn-success upload-btn" title="<?php echo trans('Upload_file'); ?>"><i class="rficon-upload"></i> <?php echo trans('Upload_file'); ?></button>
                                        <?php }
                                        ?>
                                        <?php if ($config['create_text_files']) { ?>
                                            <button class="tip btn create-file-btn" title="<?php echo trans('New_File'); ?>"><i class="icon-plus"></i><i class="icon-file"></i>
                                            </button>
                                        <?php }
                                        ?>
                                        <?php if ($config['create_folders']) { ?>
                                            <button class="tip btn new-folder" title="<?php echo trans('New_Folder') ?>"><i class="icon-plus"></i><i
                                    ...
                                    <div class="span2 half view-controller">
                                        ...
                                        ?>" id="view0" data-value="0" title="<?php echo trans('View_boxes'); ?>"><i class="icon-th <?php
                                                if ($view == 0) {
                                                    echo "icon-white";
                                                }
                                                ?>"></i></button>
                                        ...
                                        ?>" id="view1" data-value="1" title="<?php echo trans('View_list'); ?>"><i class="icon-align-justify <?php
                                                if ($view == 1) {
                                                    echo "icon-white";
                                                }
                                                ?>"></i></button>
                                        ...
                                        ?>" id="view2" data-value="2" title="<?php echo trans('View_columns_list'); ?>"><i class="icon-fire <?php
                                                if ($view == 2) {
                                                    echo "icon-white";
                                                }
                                                ?>"></i></button>
                                    ...
                                    <div class="span6 entire types">
                                        ...
                                        <?php if ($_GET['type'] != 1 && $_GET['type'] != 3 && $config['show_filter_buttons']) { ?>
                                            <?php if (count($config['ext_file']) > 0 or false) { ?>
                                                ...
                                                <label id="ff-item-type-1" title="<?php echo trans('Files'); ?>" for="select-type-1" class="tip btn ff-label-type-1"><i
                                                ...
                                                <label id="ff-item-type-2" title="<?php echo trans('Images'); ?>" for="select-type-2" class="tip btn ff-label-type-2"><i
                                                ...
                                                <label id="ff-item-type-3" title="<?php echo trans('Archives'); ?>" for="select-type-3" class="tip btn ff-label-type-3"><i
                                                ...
                                                <label id="ff-item-type-4" title="<?php echo trans('Videos'); ?>" for="select-type-4" class="tip btn ff-label-type-4"><i
                                                ...
                                                <label id="ff-item-type-5" title="<?php echo trans('Music'); ?>" for="select-type-5" class="tip btn ff-label-type-5"><i
                                        ...
                                        <input accesskey="f" type="text" class="filter-input <?php echo(($_GET['type'] != 1 && $_GET['type'] != 3) ? '' : 'filter-input-notype'); ?>"
                                               id="filter-input" name="filter" placeholder="<?php echo fix_strtolower(trans('Text_filter')); ?>..." value="<?php echo $filter; ?>"/><?php if ($n_files > $config['file_number_limit_js']) { ?><label id="filter" class="btn"><i
                                                    class="icon-play"></i></label><?php }
                                            ?>

                                        <input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all"
                                               class="hide"/>
                                        <label id="ff-item-type-all" title="<?php echo trans('All'); ?>" <?php if ($_GET['type'] == 1 || $_GET['type'] == 3) { ?>style="visibility: hidden;" <?php }
            ...
            <div class="row-fluid">
                ...
                <ul class="breadcrumb">
                    ...
                    <?php if ($config['show_total_size']) { ?>
                        <li><small class="hidden-phone"><span title="<?php echo trans('total size') . $config['MaxSizeTotal']; ?>"><?php
                                    echo trans('total size') . ": " . makeSize(
                                            $sizeCurrentFolder
                                    ) . (($config['MaxSizeTotal'] !== false && is_int(
                                            $config['MaxSizeTotal']
                                    )) ? '/' . $config['MaxSizeTotal'] . ' ' . trans('MB') : '');
                                    ?></span></small>
                        </li>
                    <?php }
                    ?>
                                        ...
                                        ?>">
                                            ...
                                            <a href="javascript:void('')" class="link" data-file="<?php echo $file; ?>" data-function="<?php echo $apply; ?>">
                                                ...
                                                <div class="img-precontainer-mini <?php if ($is_img) echo 'original-thumb' ?>">
                                                    ...
                                                    <?php }
                                                    ?>
                                                    <div class="filetype <?php echo $file_array['extension'] ?> <?php
                                                    if (in_array(
                                                                    $file_array['extension'],
                                            ...
                                            <figcaption>
                                                <form action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
                                                    ...
                                                    <a title="<?php echo trans('Download') ?>" class="tip-right" href="javascript:void('')" <?php if ($config['download_files']) echo "onclick=\"$('#form" . $nu . "').submit();\"" ?>><i
...
<?php endforeach; ?>
    ...
    <?php if ($config['tui_active']) { ?>
        ...
        <script>
            var tuiTheme = {
    <?php
    foreach ($config['tui_defaults_config'] as $aopt_key => $aopt_val) {
        if (!empty($aopt_val)) {
    ...
    ?>
        ...
        <script>
            if (image_editor) {
                ...
                $('.tui-image-editor-header-buttons div').replaceWith('<button class="tui-image-editor-exit-btn" ><?php
    echo trans(
            'Image_Editor_Exit'
    );
    ?></button>');
                ...
                $('.tui-image-editor-download-btn').replaceWith('<button class="tui-image-editor-save-btn" ><?php
    echo trans(
            'Image_Editor_Save'
    );
    ?></button>');
